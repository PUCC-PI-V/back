...
SOURCE_DATA_PATH = os.getenv("SOURCE_DATA_PATH", os.path.join(BASE_DIR, "database/vectorDatabase/source_data.txt"))
...
def build_prompt(context, input):
    return [
        {
            "role": "system",
            "content": (
...
async def prompt_index(request: Request):
    ...
    try:
        tokenizar, model, collection = load_resources()
    ...
    try:
        ...
        answer = rag_chain(question, tokenizar, model, collection)
